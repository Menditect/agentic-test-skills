...
version: "4.4.0"
changes: "Added dual-track testing strategy selection (Option A Exploratory vs Option B Persistent MTA) and promotion handoffs."
---
...
*   **⚡ Execution Strategy Decision Flow (Post-Gate 1)**:
    *   **If User Selects Option A (Immediate Local Exploratory Test):**
        *   *Behavior:* Bypasses Gate 2 placement and `STATE_CONSTRUCTION`. Compiles the approved step sequence into `TCEX_RQ` and executes in-memory against the local Mendix JVM via `mta-run-analyze` in milliseconds with automatic rollback (`RollbackTcseAfterExecution = "true"`). [^PAT-56] [^PAT-58]
        *   *Execution Plan Persistence:* Plan is **NOT** stored in MTA during exploratory testing.
        *   *Promotion Bridge:* Upon successful execution, the user can choose to promote the test to the MTA Platform with one click. [^PAT-57]
    *   **If User Selects Option B (Direct Persistent MTA Test):**
...
You **MUST** output the final approved Execution Plan inside this exact standard markdown blueprint format, including the pre-filled **Session Compaction Block**, to guarantee seamless state bootstrapping:
...
```json
{
  ...
  "Context": "[Execution Plan approved for Components Under Test | Exploratory Execution Plan ready for in-memory execution]"
}
```
```
...
*   **[CHECK 10] Dual-Track Execution Strategy Explicit Declaration (`PAT-60`)**: Verified that Section 1 explicitly declares the Execution Strategy (Option A vs Option B) and Gate 1 prompt presents both execution track options for user choice. ➔ **[PASS]**
```
...
* **If Option A (Local Exploratory Test) was approved:**
  > 🚀 **Handoff Trigger (Exploratory Track)**: Ready to transition to `mta-run-analyze`. Load the `mta-run-analyze` skill to execute the exploratory test in-memory via `MTA_plugin.execute-testcase`.
