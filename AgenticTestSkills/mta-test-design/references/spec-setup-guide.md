...
*   **🚨 Read-Only Tools Always Authorized:** You are always authorized to call read-only MTA `Get*` tools (`GetApplicationDetails`, `GetTestConfigurationDetails`, `GetAppModelData`, `GetExecutionUsers`, `GetTestSuiteDetails`, `GetTestCaseDetails`, `GetTeststepDetails`) to discover model state and existing containers.
...
*   **🚨 Execution Plan Local Storage & Provenance Gate (MANDATORY):** Immediately upon receiving Gate 2 approval, save the approved plan locally as a `.md` file (e.g. `${MTA_OUTPUT_PATH}/execution-plans/EP_<TestCaseName>.md`) sealed with YAML frontmatter (semantic `plan_id` `<TestCaseName>-v<revision>` or UUID v4, `supersedes_plan_id`, integer `revision`, ISO 8601 `approved_at`). Automatically archive prior versions (`archive/EP_<TestCaseName>_<timestamp>.md`) on revision, avoid duplicate archives if unchanged, notify the user with a clickable sealed receipt (or dual receipt), and persist metadata in `mta_state.json`. If local write tools are unavailable (Chat Track), warn the user, generate the semantic plan_id/timestamp, and retain the plan in active chat context.
...
*   **🚨 Execution Plan Approval, Drift Check & Storage Gating Rule:** You are strictly prohibited from executing active construction steps unless Gate 1 and Gate 2 are approved, the Execution Plan is saved locally as a `.md` file (or confirmed retained in active chat context if write tools are unavailable), and pre-construction drift detection confirms plan ID, revision, and approval timestamp integrity (or reconciles intentional changes).
...
*   **Atomic Sequential Provisioning:** Construct test containers and steps sequentially in forward chronological order passing resolved predecessor keys.
*   **Step Option Binding & Assertions:** Configure step attributes, parameters, bindings, and embedded assertions using `CreateObjectActionTestStep`, `EditAttributeValue`, `EditMicroflowParameterValue`, and `EditAssert*`.
...
*   **Mandatory Halt Gate:** Perform a **100% Full-Content Audit of ALL 8 Sections of the saved Execution Plan** (auditing against the local `.md` file or active chat context, and running `GetTestCaseDetails`, `GetTestSuiteDetails`).
