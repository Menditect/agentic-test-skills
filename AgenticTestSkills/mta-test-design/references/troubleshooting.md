...
| `Cannot set enumeration for attribute value, because FilterComparisonOperator is not given` | Omitted `FilterComparisonOperator` on `EditAttributeValueFilter`. | Pass singular `FilterComparisonOperator: "Equal"` (or target operator: `"NotEqual"`, `"Contains"`, etc.). |
| `Cannot set ... for assert ... compare (...), because the given ComparisonOperator is not valid` | Called value setter (`SetDecimalValue`, `SetIntegerLongValue`, `SetStringValue`) on `EditAssertMicroflowReturnValueCompare` without passing `ComparisonOperator`. | Always pass singular `ComparisonOperator: "Equal"` (or target operator) in the same call as `EditAction`. |
| `given ComparisonOperator is not valid` or `Value '...' is not valid for ComparisonOperator` | Used wrong singular/plural casing or guessing count syntax. | Consult the Assertion & Filter Operator Matrix in `api-helpers.md`: singular `Equal`/`NotEqual` for attribute value comparisons (`EditAssertAttributeValueCompare`), microflow return comparisons (`EditAssertMicroflowReturnValueCompare`), and retrieve filters (`EditAttributeValueFilter`); plural `Equals`/`NotEquals` for microflow return creation (`CreateAssertMicroflowReturnValue`) and validation feedback messages; mixed format (`Equals`, `Greater_than`, `GreaterThanEqualTo`, `Less_than`, `LessThanEqualTo`) for count assertions. |
