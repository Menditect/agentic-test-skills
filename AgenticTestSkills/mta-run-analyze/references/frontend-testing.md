...
*   **The Cross-Case Dynamic Scalar Value Piping Law (Dynamic Data & Retrieve Rule):** Whenever page widgets interact with or assert data seeded in Case 1—including `ComboBox`, `ReferenceSelector`, and `ReferenceSetSelector` dropdowns, text inputs (`ACT_Fill_TextInput_Input`), search/filter inputs (`ACT_Fill_SearchInput`, `ELO_Filter_*_by_Text`), and text assertions (`ACT_Assert_ElementText_Equal`)—you **MUST** prioritize using **Dynamic Scalar Value Piping** (`SelectValueForValue`) referencing the upstream seed step's `TestStepOutputKey` and attribute name instead of hardcoding static literal strings.
    *   **Data Seeded in Same Suite (Case 1 ➔ Case 2):** Pipe the identifier attribute (e.g., `Name`, `Code`, `Description`, `Email`) directly from the upstream Case 1 creation step (`TestStepOutputKey`).
    *   **Pre-existing / External Master Data:** If referencing static external master data (e.g. Countries, Currencies) not created in the same test suite, explicitly add a retrieve-from-database teststep early in Case 1 with an explicit attribute filter, and use that retrieve step's output as the provider (`TestStepOutputKey`) for downstream scalar piping.
*   **The Self-Contained Seeding & Master Data Invariant (CRITICAL) (`PAT-91`, `ANTI-42`):** Frontend UI test suites MUST be fully autonomous and self-contained across clean or containerized environments.
...
6.  **Dynamic Scalar Value Piping:** Use dynamic scalar value piping (`SelectValueForValue`) referencing upstream Case 1 seed data handles for all page interactions consuming seeded attributes—including selection widgets (dropdowns, comboboxes, reference selectors), text inputs (`ACT_Fill_TextInput_Input`), search/filter fields (`ACT_Fill_SearchInput`, `ELO_Filter_*_by_Text`), and UI text assertions (`ACT_Assert_ElementText_Equal`)—instead of hardcoding static literal strings.
...
1. **Persistent 3-Case Suite Lifecycle (`PAT-03`):** Frontend UI tests are constructed across 3 dedicated test cases: Case 1 (Setup Test Case: database seeding via `Create Object` + batch `Persist` with `"Always"` condition, browser launch with login/anonymous navigation [`PAT-91`]), Case 2 (Action Test Case: TestKit UI interactions and assertions), and Case 3 (Teardown Test Case: browser shutdown via `Stop_MxFrontendTest` with `"Always"` condition, symmetric database cleanup via `Retrieve Object` + `Delete Object` in reverse dependency order [`PAT-93`] followed by a mandatory trailing batch `Persist` step (`ObjectAction = "Persist"`) with `ExecutionCondition = "Always"` and `ResumeExecutionAfterException = "_Continue"` [`PAT-92`, `ANTI-43`]).
