...
* **Scope:** General | **Classification:** Methodological Law
* **Description:** Enforces a strict 8-field schema in exact order for specifying every test step in Execution Plans: `Step Type`, `Target / Entity / Action`, `Input Source / Handles`, `Output Variable Handle`, `Parameters & Attribute Values`, `Embedded Step Assertions`, `Execution Settings`, and `Step Description & Pattern Rationale`. All 8 fields are explicitly populated in the detailed step drilldown cards (`### Detailed Step Configurations & Assertions`). The high-level `Step Sequence Matrix` overview table and executive summary step tables present the concise 7 operational columns, delegating narrative description and pattern citations cleanly to the collapsed cards and Section 8.
* **Related Rules:**
  * **Related Patterns:** `PAT-12` (Test Step Description Pattern Annotation).
...
* **Scope:** General | **Classification:** Methodological Law
* **Description:** When initiating test design for a target microflow or page, the agent MUST silently search the local execution plans directory (`${MTA_OUTPUT_PATH}/execution-plans/`) for any existing `EP_*.md` files targeting the same element before drafting a new plan. If a prior plan is discovered, the agent must perform an AST delta audit comparing the current live AST against the verified elements in Section 4 of the prior plan to identify changed signatures, parameters, or domain models. The agent must present an interactive Lineage Decision Card in chat adhering strictly to the Scoped Note Box & Clean Markdown Standard: (1) The `> [!NOTE]` callout box is strictly isolated to the discovery header line and metadata bullet points (including a 1–2 line AST delta summary); (2) The canonical lineage choices—Path 0: Use Existing Plan As-Is (Fast-Path, recommended when in-sync with 0 AST delta), Path A: Evolve & Supersede, Path B: Branch Companion Case, and Path C: Clean Slate—are rendered in a clean, focused 4-column Markdown table directly beneath the note box; (3) All ASCII/Unicode box-art characters (`╔`, `═`, `║`, `╠`, `╚`, `┌`, `─`, `│`, `└`), terminal borders, and multi-table cascades (such as dumping URL tables or 5-column AST matrices in chat) are strictly PROHIBITED; (4) Inventing unverified random paths is strictly PROHIBITED. Selecting Path 0 loads the active plan as the SSOT without modifying the file or bumping revisions, skipping directly to Gate 2 placement or execution.
* **Related Rules:**
...
* **Scope:** General | **Classification:** Methodological Anti-Pattern
* **Description:** Blindly overwriting an existing execution plan file on disk, discarding prior test specifications with complete context amnesia when re-testing or iterating on a microflow or page, or dumping unreadable ASCII-art borders (`╔═...═╗`) and multi-table cascades into chat during prior plan discovery. This anti-pattern causes loss of carefully crafted domain edge cases, risk profiles, and historical test rationale, results in accidental duplicates on the MTA platform with confusing revisions, or overwhelms the user with fragmented terminal borders and table sprawl.
* **Related Rules:**
...
* **Scope:** General | **Classification:** Methodological Law
* **Description:** Mandates that during test design (`STATE_BUILD_PLANNING` / `PLAN_STEP_1`), the full, exhaustive 8-section Execution Plan MUST be persisted directly to disk at `${MTA_OUTPUT_PATH}/execution-plans/EP_<TestCaseName>.md` with `status: "DRAFT"` and outer collapsible headers (`<details><summary><b>...</b></summary>`) across all major sections (Metadata, Pre-Approval Checklist, and Sections 1 through 8). Conversational chat output MUST be strictly limited to an Executive Plan Summary Box, the pre-approval quality audit status, a clickable file link to the `.md` file, and the Checkpoint 1 Strategy Decision Card. The "Executive Summary Box" is a semantic concept representing a clean, compact native Markdown blockquote or list (~35 lines) and MUST NEVER be rendered using ASCII or Unicode box-drawing characters (`╔`, `═`, `║`, `╚`, `┌`, `─`, `│`, `└`). The internal `Pattern Applicability Checklist` is relegated exclusively to silent internal reasoning tokens and MUST NOT be output in conversational chat.
* **Related Rules:**
...
### `PAT-90`: Playwright Tracefile & Interactive Viewer Integration Law
* **Scope:** Frontend | **Classification:** Platform Execution Law
* **Description:** When executing frontend UI tests where Playwright tracing is enabled, MTA captures execution traces and stores them on the server, returning a `FileUUID` in the test run receipt (`GetTestRunResults`). This law mandates resolving `playwright_viewer_url` (default `https://trace.playwright.dev/?trace=`) and `tracefile_base_url` (from `mta_config.json`, `.env`, or derived from `${mta_base_url}/rest/private/tracefile?fileUUID=`) to assemble and render a clickable interactive trace viewer link in failure diagnostics and test run receipts. This allows testers and AI agents to visually inspect DOM snapshots, step execution timelines, console messages, and network calls without needing local browser tools.
* **Related Rules:**
  * **Related Patterns:** `PAT-05` (Menditect Frontend Testkit Strict Default Law), `PAT-28` ("Start-and-Stop First" Boilerplate Session Law), `PAT-83` (Context-Preserving Diagnostic Drill-Down Law).
