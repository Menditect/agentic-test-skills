...
*   **🚨 Execution Plan Local Storage & Provenance Gate (MANDATORY):** Immediately upon receiving Gate 2 approval, save the approved plan locally as a `.md` file (e.g. `${MTA_OUTPUT_PATH}/execution-plans/EP_<TestCaseName>.md`) sealed with YAML frontmatter (UUID v4 `plan_id`, `supersedes_plan_id`, integer `revision`, ISO 8601 `approved_at`). Automatically archive prior versions (`archive/EP_<TestCaseName>_<timestamp>.md`) on revision, avoid duplicate archives if unchanged, notify the user with a clickable sealed receipt (or dual receipt), and persist metadata in `mta_state.json`. If local write tools are unavailable (Chat Track), warn the user, generate the UUID v4/timestamp, and retain the plan in active chat context.
