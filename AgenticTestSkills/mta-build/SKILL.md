...
version: "4.8.5"
changes: "Enforced PAT-77 (Mandatory Data Variation Container Metadata & Description Persistence Law) and ANTI-31 during container creation and smoke audit."
---
...
2. **No parallel or batched creations [^PAT-16] [^ANTI-05]**: Create cases and steps sequentially, waiting for Key N's response before building step N+1.
...
11. **🚫 STRICT DATA VARIATION PROMOTION & EXHAUSTIVE CELL RECONCILIATION LAW [^PAT-19] [^PAT-27] [^PAT-54] [^PAT-77] [^ANTI-08] [^ANTI-11] [^ANTI-31]**: 
    ...
    *   **Mandatory Variation Container Metadata & Description Persistence (`PAT-77`, `ANTI-31`):** When creating or promoting data variations, you **MUST** configure both the Name and Description on the MTA server for every variation container:
        1. *Template Variation (Scenario #1):* Upon calling `EnableTestCaseDataVariations`, immediately call `TestCaseDataVariationName(TemplateVariationKey, Name)` AND `TestCaseDataVariationDescription(TemplateVariationKey, Description)`.
        2. *Duplicated Variations (Scenarios #2..#N):* For each column created via `DuplicateTestCaseDataVariation`, immediately call `TestCaseDataVariationName(NewVariationKey, Name)` AND `TestCaseDataVariationDescription(NewVariationKey, Description)` matching Section 7 of the Execution Plan. Leaving descriptions empty or unpersisted on the server is strictly prohibited (`ANTI-31`).
    *   **Exhaustive $M \times N$ Matrix Cell Reconciliation Law (`PAT-54`, `ANTI-11`):** Calling `DuplicateTestCaseDataVariation` (or `DuplicateTestSuiteDataVariation`) ONLY allocates a new variation column container. It DOES NOT set scenario values. You are strictly prohibited from performing "delta-only overrides" or assuming copied columns inherit correct values (`ANTI-11`). After duplicating columns and setting container metadata (`PAT-77`), you MUST call `GetTestCaseDataVariationsDetails`, iterate through **EVERY SINGLE CELL $(i, j)$** in the $M \times N$ matrix, compare the live MTA value against the Execution Plan matrix, and explicitly call the setter tool for every cell that does not match (`PAT-54`).
...
17. **Incremental Construction Success Verification Checklist [^PAT-49]**:
...
23. **Reactive Model Revision Mismatch Diagnostic & Interception Law [^PAT-59] [^ANTI-17] [^ANTI-18]**:
    *   **The Guardrail:** If any mutating step creation tool (e.g. `CreateTestStepCreateObject`, `SetStringAttributeValue`, `CreateMicroflowCallTestStep`, `SetSelectObjectFor*`) fails with a model element not-found or binding error, you **MUST IMMEDIATELY HALT** further step creation. Continuing to call subsequent step tools is strictly prohibited (`ANTI-18`).
    *   **The Action:** Diagnose and explain the failure to the user: relate the error to an MTA Model Revision mismatch (identifying the specific missing entity, attribute, microflow, parameter, or widget), inform the user that further persistent step building cannot proceed until the MTA Model Revision is upgraded, and for Backend logic offer immediate in-memory exploratory testing via `MTA_plugin.execute-testcase` to test the uncommitted local logic against the running Mendix runtime in the meantime.
...
| *Auditing step sequences, validating all 95 testing patterns/anti-patterns (`PAT-01..70`, `ANTI-01..25`), auto-registering new learned patterns* | **`references/mta-patterns-and-antipatterns-reference.md`** |
...
2. **Phase 2: `STATE_CONSTRUCTION`**
   ...
   - **Reactive Revision Mismatch Interception:** If any tool fails with a model element not-found error, immediately halt further step creation (`ANTI-18`), identify the missing element, relate it to an MTA Model Revision mismatch (`PAT-36`, `PAT-59`), explain why further building cannot proceed, and offer local exploratory testing via `MTA_plugin.execute-testcase`.
   - **Mode-Specific Execution Style:**
     *   **Agentic Mode:** Call programmatic MCP tools (such as `CreateTestStep`, `SetBooleanAttributeValue`, `CreateTestStepPersist`, etc.) sequentially in the background. Output a clear `🧠 Tool Execution Reasoning` markdown explanation before every single call. Wait for Key N's response before initiating Step N+1.
...
3. **Phase 3: `STATE_SMOKE_AUDIT`**
   ...
   - *Milestone:* Run the programmatic server compiler checks (`GetTestConstructionErrorsOfTestCase`), enforce the **Zero Construction Error Gate** (`PAT-59`, `ANTI-18`), perform a **100% Full-Content Audit of ALL 8 Sections of the saved Execution Plan** (`GetExecutionPlan`, `GetTestSteps`, `GetTestCaseDataVariationsDetails` / `GetTestSuiteDataVariationsDetails`), and output the Post-Construction Smoke Audit Report (unlocked only if a valid `ExecutionPlanKey` is present).
   - **Zero Construction Error Mandatory Gate:** Invoking `GetTestConstructionErrorsOfTestCase` MUST return 0 errors. If construction errors exist, transition to `STATE_RUN_ANALYZE` is strictly blocked until errors are resolved.
   - **Mandatory 8-Section Plan Conformity Audit:**
     ...
     7. **Section 7 (Data Variation Matrix & Metadata):** **Cell-by-cell & item-by-item audit**: Call `GetTestCaseDataVariationsDetails` (or `GetTestSuiteDataVariationsDetails`) and verify system names, non-empty descriptions matching Section 7 (`PAT-77`, `ANTI-31`), input attribute/parameter overrides, return value assertions, object counts, exception strings, and validation feedback strings against Section 7 matrix (`PAT-54`).
