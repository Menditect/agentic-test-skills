...
version: "4.4.0"
changes: "Added exploratory execution guide, in-memory test promotion workflows, and embedded assertion updates."
---
...
15. **Allowed Operational States for Parameter Setters & AALC Assertions [^PAT-48]**:
...
16. **Incremental Construction Success Verification Checklist [^PAT-49]**:
...
23. **Reactive Model Revision Mismatch Diagnostic & Interception Law [^PAT-59] [^ANTI-17] [^ANTI-18]**:
    ...
    *   **The Action:** Diagnose and explain the failure to the user: relate the error to an MTA Model Revision mismatch (identifying the specific missing entity, attribute, microflow, parameter, or widget), inform the user that further persistent step building cannot proceed until the MTA Model Revision is upgraded, and offer immediate in-memory exploratory testing via `MTA_plugin.execute-testcase` to test the uncommitted local logic against the running Mendix runtime in the meantime.
...
| *Auditing step sequences, validating all 77 testing patterns/anti-patterns (`PAT-01..59`, `ANTI-01..18`), auto-registering new learned patterns* | **`references/mta-patterns-and-antipatterns-reference.md`** |
| *Promoted exploratory tests, TCEX_RQ to MTA construction transformer* | **`references/mta-plugin-mcp-schema.md`** |
