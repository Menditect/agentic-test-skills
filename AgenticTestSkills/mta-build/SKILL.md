...
version: "6.2.0"
changes: "Refactored workflow for removed SaveExecutionPlan/GetExecutionPlan tools; integrated local execution plan markdown storage and chat-context fallback; updated PAT-44."
---
...
2. **Two-Phase Skeleton & Batch Binding Law [^PAT-16] [^PAT-78] [^ANTI-05]**: Phase 1 (`SKELETON_PROVISIONING`): Sequentially provision containers and empty test steps forward using predecessor chaining (`TestStepBeforeKey`). Phase 2 (`BATCH_BINDING`): Concurrently dispatch all parameter/attribute setters, associations, and dedicated output binding tools (`PAT-80`) in massive parallel arrays in a single turn (`PAT-81`).
...
5. **Atomic Multi-Case Session Construction & Execution Plan Gating [^PAT-44]**: In UI or multi-case tests, do not halt sequentially for browser setup or separate cases. Before entering `STATE_CONSTRUCTION`, ensure the approved Execution Plan is saved locally as a `.md` file (or retained in active chat context with a warning if write tools are unavailable) upon receiving Gate 2 approval at the end of `STATE_BUILD_PLANNING`. Upon entering `STATE_CONSTRUCTION` with this active execution plan context, atomically construct all containers, set up browser options, and build chronological steps in a single execution sweep.
...
2. **Phase 2: `STATE_CONSTRUCTION`**
   - *State Header:* `[State: STATE_CONSTRUCTION | Temp State: [SKELETON_PROVISIONING | BATCH_BINDING] | Active Skill: mta-build]`
   ...
   - **Mode-Specific Execution Style:**
     *   **Agentic Mode:** Execute the Two-Phase Skeleton & Batch Binding loop (`PAT-78`). 
         1. Set `Temp State: SKELETON_PROVISIONING`. Concurrently dispatch `CreateTestCase` for all cases (`PAT-79`). Sequentially create all empty test steps using predecessor chaining (`PAT-11`).
         2. Set `Temp State: BATCH_BINDING`. Unleash the configuration tools. Concurrently dispatch all setters across Variations, Suite properties, Case specs, and Steps (`EditAttributeValue`, `EditMicroflowParameterValue`, `CreateAssert*`, `CreateSelectObjectForAssociation`, `SetTestStepOutputFor*`) in massive parallel arrays using the resolved keys. Output a clear `🧠 Tool Execution Reasoning` markdown explanation before every single call.
...
3. **Phase 3: `STATE_SMOKE_AUDIT`**
   ...
   - *Milestone:* Audit saved Execution Plan against local `.md` file (or chat context), audit created test cases and steps (`GetTestCaseDetails`), verify 0 construction discrepancies, and output the Post-Construction Smoke Audit Report.
   - **Mandatory 8-Section Plan Conformity Audit:**
     1. **Section 1 (Metadata & Placement):** App, Config, Suite, Case Name, Category, Execution User (`GetExecutionUsers`).
   ...
   - **Mode-Specific Execution Style:**
     *   **Agentic Mode:** Read the local Execution Plan markdown file (at `${MTA_OUTPUT_PATH}/execution-plans/EP_<TestCaseName>.md` or path in `mta_state.json`) to retrieve the approved specifications. Call `GetTestCaseDetails(TestCaseKey)` to audit steps and variations line-by-line. Generate and output the Post-Construction Smoke Audit Report, including direct clickable MTA Web navigation links (`[MtaBaseUrl]/p/[ObjectType]/[Key]`) for target Config, Suite, and Case(s), along with a link to the local Execution Plan file.
