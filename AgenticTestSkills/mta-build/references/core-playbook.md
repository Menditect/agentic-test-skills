...
4.  **`STATE_SMOKE_AUDIT`**: Performs structural checks, verifies plan ID and revision integrity, runs programmatic validations via staggered reading (`GetTestCaseDetails` one test case per turn), executes mandatory **1-to-1 Plan-to-Server Step Reconciliation** (`PAT-59`, `PAT-88`), and generates the Post-Construction Smoke Audit Report featuring the Step Reconciliation Table and direct MTA navigation links (`[MtaBaseUrl]/p/[ObjectType]/[Key]`). User approves.
