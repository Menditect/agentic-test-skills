...
| **`EditAssertMicroflowReturnValueCompare`** | `ComparisonOperator` | **Singular**: `"Equal"`, `"NotEqual"`, `"GreaterThan"`, `"GreaterThanOrEqual"`, `"LessThan"`, `"LessThanOrEqual"`, `"Contains"`, `"NotContains"`, `"StartsWith"`, `"EndsWith"`<br>*(MANDATORY: Must pass `ComparisonOperator` alongside value setters like `SetDecimalValue`)* |
