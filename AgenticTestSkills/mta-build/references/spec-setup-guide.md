...
*   **🚨 Execution Plan Local Storage Gate (MANDATORY):** Immediately upon receiving Gate 2 approval, save the approved plan locally as a `.md` file (e.g. `${MTA_OUTPUT_PATH}/execution-plans/EP_<TestCaseName>.md`). If local write tools are unavailable (Chat Track), warn the user and retain the plan in active chat context.
...
*   **🚨 Execution Plan Approval & Storage Gating Rule:** You are strictly prohibited from executing active construction steps unless Gate 1 and Gate 2 are approved and the Execution Plan is saved locally as a `.md` file (or confirmed retained in active chat context if write tools are unavailable).
