...
* **Scope:** General | **Classification:** Methodological Law
* **Description:** When initiating test design for a target microflow or page, the agent MUST silently search the local execution plans directory (`${MTA_OUTPUT_PATH}/execution-plans/`) for any existing `EP_*.md` files targeting the same element before drafting a new plan. If a prior plan is discovered, the agent must perform an AST delta audit comparing the current live AST against the verified elements in Section 4 of the prior plan to identify changed signatures, parameters, or domain models. The agent must present an interactive Tri-Choice Lineage Decision Card in chat adhering strictly to the Scoped Note Box & Clean Markdown Standard: (1) The `> [!NOTE]` callout box is strictly isolated to the discovery header line and metadata bullet points (including a 1–2 line AST delta summary); (2) The 3 canonical lineage choices (Path A: Evolve & Supersede, Path B: Branch Companion Case, Path C: Clean Slate) are rendered in a clean, focused 4-column Markdown table directly beneath the note box; (3) All ASCII/Unicode box-art characters (`╔`, `═`, `║`, `╠`, `╚`, `┌`, `─`, `│`, `└`), terminal borders, and multi-table cascades (such as dumping URL tables or 5-column AST matrices in chat) are strictly PROHIBITED; (4) Inventing unverified paths (such as 'Path D') is strictly PROHIBITED.
* **Related Rules:**
