...
*   **The Predecessor Omission Rule (First Element in Empty Container):** For the absolute first element (step or case) in an empty test case or empty test suite, you **MUST** completely omit the predecessor parameter (`TestStepBeforeKey` or `TestCaseBeforeKey`) from the tool call. 
    *   **Prohibition:** Do **NOT** pass `0`, `null`, or any other placeholder value for the first element. Passing `0` or `null` is strictly prohibited under normal circumstances.
    *   ⚠️ **TEMPORARY SCHEMA VALIDATION WORKAROUND (TRUST THE SKILL):** If the active client-side MCP tool schema has not yet been updated and marks `TestStepBeforeKey` or `TestCaseBeforeKey` as a **required** parameter (which throws an immediate client validation error if omitted), you MUST override the omission rule and pass `0` as the fallback placeholder value. The MTA backend is designed to interpret `0` as the absolute first position. This allows you to satisfy schema validation constraints while trusting and adhering to the placement logic defined by this skill.
*   **The Non-Empty Container Predecessor Sub-Rule (Subsequent Elements):** For non-empty suites or cases, you **MUST** first query the existing elements (via `GetTestCases` or `GetTestSteps`) to retrieve the last element's key, using it as the predecessor key to append chronologically. For subsequent elements created within the same turn cycle, you MUST use the actual non-zero numeric key returned by their immediate predecessor to chain them forward chronologically (e.g., Step B uses Step A's returned key as `TestStepBeforeKey`).

```
Create Step A ──► TestStepBeforeKey completely omitted     (KeyA returned)
...
*   **🚨 THE `SetSequenceOfTeststep` SAFEGUARDS:** When using this tool to manually update step sequences, you MUST adhere to three strict safety gates:
    ...
    3.  *First-Position Sequencing Pattern:* In the updated schema, `TestStepBeforeKey` is officially **optional** for `SetSequenceOfTestStep`. To sequence an existing teststep to the absolute first position of a testcase, call `SetSequenceOfTestStep` with the target `TestStepKey` and completely omit the `TestStepBeforeKey` parameter.
*   **🚨 THE `SetSequenceOfTestCase` SAFEGUARDS:** When sequencing test cases within a test suite:
    1.  *First-Position Pattern:* Symmetrically, `TestCaseBeforeKey` is **optional** in `SetSequenceOfTestCase`. To sequence a testcase to the absolute first position of a suite, call `SetSequenceOfTestCase` with the target `TestCaseKey` and completely omit `TestCaseBeforeKey` from the tool call.
...
*   **🔄 THE TEST STEP REORGANIZATION RULE (`MoveTestStepToOtherTestCase`):**
    ...
    2.  *Target Placement:* Use the optional `TestStepBeforeKey` parameter to specify where inside the destination testcase the step should reside. Symmetrically, to place the moved step at the absolute beginning of the target testcase, completely omit `TestStepBeforeKey` from the tool call.
...
*   **🚨 THE MANUAL INTERVENTION HIGHLIGHT PROTOCOL (`SetHighlightOfTestStep`):** When you cannot fully automate astep as planned due to system limitations, lack of tool support, or because manual configuration is required, you MUST implement a placeholder step, highlight it to make it stand out (the official highlight/mark color is **blue**), and suggest manual finishing.
    *   *Lack of Tool Support (e.g., Deletion Actions):* Since backend delete operations (such as Mendix database deletion actions) are not supported by the MCP server and require manual completion in MTA, any such placeholder teststep MUST be highlighted (colored **blue**) by calling `SetHighlightOfTestStep(TestStepKey=Key, Highlight=true)`.
...
4.  **Test Case Idempotency & Lifecycle-Based Data Cleanup:**
    *   **Industry Standard:** Tests must be idempotent (runnable in any order, repeatedly, concurrently, or in isolation) without leaving leftover "dirty data" that pollutes the database or causes cascading test failures.
        *   **Dynamic Data Seeding & Teardown inside the Execution Case (Case 2):** Create all necessary dynamic test data at the start of the **Execution testcase** (Case 2) and delete them at the end of the same testcase (Case 2).
        *   **🚨 THE "START-AND-STOP FIRST" BOILERPLATE CREATION RULE (Category B):** 
            ...
            *   **🚨 The Persist Rule (Separate Sessions & Single Persist Batching):** Because the backend technical runner's session and the frontend browser session are completely unshared, the browser *cannot* see backend-seeded data unless it is written to the database. Therefore:
                1.  **For Seeding:** You **MUST** call a `Persist` step immediately after your seeding block to write the data to the database.
                2.  **For Teardown:** You **MUST** call a `Persist` step immediately after your deletion block to commit the deletions.
