...
* **`Frontend` Scope (17 Rules: 15 Patterns, 2 Anti-Patterns):** Governs browser/Playwright automation, Menditect Frontend Testkit widgets (`ACT_Fill_*`, `ACT_Click_*`), UI locators, repeating container ELO filters, browser session lifecycles (`Start`/`Stop`), page navigation, and the 3-case split.  
  *(Rules: `PAT-03`, `PAT-05`, `PAT-13`, `PAT-18`, `PAT-20`, `PAT-22`, `PAT-28`, `PAT-29`, `PAT-35`, `PAT-36`, `PAT-40`, `PAT-41`, `PAT-42`, `PAT-50`, `PAT-52`, `ANTI-12`, `ANTI-14`)*
* **`General` Scope (35 Rules: 29 Patterns, 6 Anti-Patterns):** Overarching testing principles, MTF testing pyramid scoping, MTA MCP tool execution quirks (predecessor chaining, key persistence), state machine gating, step naming standards, and multi-scenario Data Variation matrices.  
  *(Rules: `PAT-01`, `PAT-02`, `PAT-11`, `PAT-12`, `PAT-15`, `PAT-16`, `PAT-19`, `PAT-21`, `PAT-24`, `PAT-25`, `PAT-27`, `PAT-30`, `PAT-32`, `PAT-33`, `PAT-34`, `PAT-37`, `PAT-38`, `PAT-39`, `PAT-43`, `PAT-44`, `PAT-45`, `PAT-46`, `PAT-47`, `PAT-48`, `PAT-49`, `PAT-51`, `PAT-53`, `PAT-54`, `PAT-55`, `ANTI-02`, `ANTI-04`, `ANTI-05`, `ANTI-08`, `ANTI-09`, `ANTI-11`)*
...
* **Domain E: Frontend UI & Locator Laws** (`PAT-05`, `PAT-13`, `PAT-28`, `PAT-35..36`, `PAT-40..42`, `ANTI-12`)
* **Domain F: Protocol Gates, State Machine & Multi-Agent Safety** (`PAT-04`, `PAT-10`, `PAT-43..53`, `ANTI-13..14`)
...
| Rule ID | Pattern / Rule Name | Category | Classification | Reference Document | Core Enforcement & Summary |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **PAT-06** | **Direct Attribute & Association Initialization on Create Object** | **Backend** | Methodological Law | [golden-rules.md](golden-rules.md) | Sets ALL initial attributes and association bindings directly on the `Create Object` step. Prohibits creating a separate `Change Object` step immediately following `Create Object`. |
| **PAT-08** | **Retrieve / Microflow Output Object Count Assertion** | **Backend** | Methodological Law | [golden-rules.md](golden-rules.md) | Inserts an `Assert Object Count` step immediately following any `Retrieve Object` or `Microflow Call` producer step before downstream consumption (excluding `Create Object`). |
| **PAT-09** | **Backend-First Delete Pattern** | **Backend** | Methodological Law | [golden-rules.md](golden-rules.md) | Executes a backend `Retrieve Object` step first before calling `Delete Object`, piping the retrieved object into the delete step. |
...
| **PAT-14** | **Prohibition of Embedded Asserts on Create/Change Object Steps** | **Backend** | Methodological Law | [golden-rules.md](golden-rules.md) | Initial attributes on Create and modified attributes on Change are mutators, NOT assertions. Embedded assertions belong on Retrieve, Microflow Call, or UI steps. |
...
| **ANTI-01** | **Separate `Change Object` Step After `Create Object`** | **Backend** | Platform Anti-Pattern | [golden-rules.md](golden-rules.md) | Creating an object and adding a separate `Change Object` step immediately after instead of initializing values directly on `Create Object`. |
| **ANTI-03** | **Unasserted Retrieve / Microflow Output Consumer Piping** | **Backend** | Methodological Anti-Pattern | [golden-rules.md](golden-rules.md) | Passing a retrieved object/list directly into downstream parameters without first validating existence via `Assert Object Count`. |
...
| **ANTI-06** | **Asserting Object Count after `Create Object`** | **Backend** | Methodological Anti-Pattern | [golden-rules.md](golden-rules.md) | Adding unnecessary `Assert Object Count` steps after a `Create Object` step (in-memory instantiated objects are guaranteed to exist). |
| **ANTI-10** | **Embedded Assertions on Create/Change Object Steps** | **Backend** | Methodological Anti-Pattern | [golden-rules.md](golden-rules.md) | Configuring assert comparisons on Create or Change object steps instead of using them purely for initialization/mutation. |
...
| **PAT-36** | **MTA Sync Probe & Local Model Fallback** | **Frontend** | Methodological Law | [frontend-testing.md](frontend-testing.md) | Inquires if MTA is up to date before running `GetPages`/`GetWidgets`; falls back to `mxcli` (`SHOW PAGES`) if not. |
...
| **PAT-42** | **Date-Time Offset & Format Pattern Inspection** | **Frontend** | Methodological Law | [frontend-testing.md](frontend-testing.md) | Uses `CurrentDateTime` with relative offset for date-time widgets and inspects `dateformPattern` in the Mendix model via `mxcli`. |
