...
version: "4.2.4"
changes: "updated read-only MTA Get tools rule, fixed ExecutionPlanKey handoff timing, and updated red lines"
---
...
> * Follow the 3-Step Interactive Planning Loop (`PLAN_STEP_1` Specs $\rightarrow$ `PLAN_STEP_2` Placement $\rightarrow$ `PLAN_STEP_3` Playwright Settings) in `mta-test-design` to produce an approved Execution Plan and execute `SaveExecutionPlan` to retrieve the `ExecutionPlanKey` before constructing steps here.
